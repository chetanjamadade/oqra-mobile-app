...
(function() {
  angular.module('ionic.rating', []).constant('ratingConfig', {
    ...
    stateOn: null,
    stateOff: null
  }).controller('RatingController', function($rootScope, $scope, $attrs, ratingConfig) {
    var ngModelCtrl;
    ngModelCtrl = {
    ...
    $scope.rate = function(value) {
      if (!$scope.readonly && value >= 0 && value <= $scope.range.length) {
        ...
        if (value == 1) {
            $rootScope.ratingtext = 'Very Dissatisfied';
            $rootScope.rating = 1;
        } else if (value == 2) {
            $rootScope.ratingtext = 'Dissatisfied';
            $rootScope.rating = 2;
        } else if (value == 3) {
            $rootScope.ratingtext = 'Satisfied';
            $rootScope.rating = 3;
        } else if (value == 4) {
            $rootScope.ratingtext = 'Very Satisfied';
            $rootScope.rating = 4;
        } else {
            $rootScope.ratingtext = 'Extremly Satisfied';
            $rootScope.rating = 5;
        }

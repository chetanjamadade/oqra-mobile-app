angular.module('starter.controllers', ['timer'])
            ...
            $http({
                ...
                .success(function(data) {
                    if (data.status) {
                        ...
                        $scope.submit.client_id = localStorageService.get('client_id');
                        $scope.submit.guest_id = localStorageService.get('guest_id');
                        $scope.submit.rating_id = localStorageService.get('rating_id');
                        $rootScope.showLoading();
                        $http({
    ...
    .controller('LoginCtrl', function(Idle,$rootScope,$scope,$cordovaNetwork,$location,$timeout,$ionicPlatform,$http, texts, locations, localStorageService, social, url, $cordovaGeolocation, configuration) {
        $http({
            url: 'http://fluohead.com/oqra-dashboard/admin/inc/functions/mobileUp.php?key=test&route=getConfiguration',
            method: 'POST',
            headers: {'Content-Type': 'application/x-www-form-urlencoded'}
        })
            .success(function (data) {
                if(data.dictionary != undefined){
                    for(var i =0; i<data.dictionary.length; i++){
                        var dictionary = [];
                        dictionary["key"] = data.dictionary[i]["key"];
                        var value = [];
                        for(var val in data.dictionary[i]){
                            if(val != "key" && val != "id"){
                                value[val] = data.dictionary[i][val];
                            }
                        }
                        dictionary["value"] = value;
                        texts.dictionary.push(dictionary);
                    }
                    $scope.getFromDictionary = function(key) {
                        var value = texts.dictionary.filter(function (x){return x.key == key })[0].value[configuration.language];
                        return value;
                    };
                }
            })
    ...
    .controller('DashCtrl', function(Idle,$rootScope,$scope,$cordovaNetwork,$location,$timeout,$ionicPlatform,$http, texts, locations, localStorageService, social, url, $cordovaGeolocation, configuration) {
        $scope.results = {};

...
.controller('RateCtrl',function(texts,$ionicHistory,$timeout,$scope,$rootScope,$http,$location,localStorageService,customer,social) {

    $http({
            url: 'http://fluohead.com/oqra-dashboard/admin/inc/functions/mobileUp.php?key=test&route=loadlocationconf&lid=' + localStorageService.get('rating_location'),
            method: 'POST',
            headers: { 'Content-Type' : 'application/x-www-form-urlencoded' },
        ...
        $cordovaGeolocation.getCurrentPosition(opts).then(function(pos) {
                ...
                .success(function(data) {
                    if (data.status == false) {
                        ...
                        $scope.results = data.results;
                        $scope.error = false;
                        localStorageService.set('rating_location',data.results[0].id);
                        if(data.results[0].total_professionals > 0){
                            $location.path('/app/professional');
                        }else {
                            $location.path('/app/rate');
                        }
